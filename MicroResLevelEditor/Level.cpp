...
bool Level::save(const char* path)
{
	...
	// Generate Lines from tiles
	for (unsigned int i = 0; i < tiles.size(); i++)
	{
		std::string data = "tile,";
		data.append(std::to_string(tiles[i].X));
		data.append(",");
		data.append(std::to_string(tiles[i].Y));
		data.append(",");
		data.append(tiles[i].Tileset);
		data.append(",");
		data.append(std::to_string(tiles[i].TX));
		data.append(",");
		data.append(std::to_string(tiles[i].TY));
		data.append(",");
		data.append(tiles[i].Passable ? "1" : "0");

		if (tiles[i].Link) // Extra Data for Linked Level
		{
			data.append(",");
			data.append(tiles[i].LinkedLevel);
			data.append(",");
			data.append(std::to_string(tiles[i].DX));
			data.append(",");
			data.append(std::to_string(tiles[i].DY));
		}
	...
	std::cout << "Begin Write to file: " << filename << std::endl;
	...
	std::cout << "Finished Writing." << std::endl << std::endl;
