...
HotdogWeapon::HotdogWeapon(const char* spritename, unsigned int row, 
	...
	AnimatedGameObject("HotdogWeapon", spritename, 2, row, 4, position + offset, boxSize, boxOffset, textureOffset)
{
...
void HotdogWeapon::Update()
{
	...
	{
		performMovement();
		if (position.X < -10.0f || position.X > 74.0f || position.Y < -10.0f || position.Y > 74.0f)
			alive = false;
